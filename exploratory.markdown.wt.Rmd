...
dept.plot <- ggplot(data = sales.by.dept2) +
  geom_segment(aes(x=1, xend=2, y=`2016`, yend=`2017`)) + 
  geom_text(aes(label=Department, y=`2016`), x=1, hjust=-0.2, check_overlap = TRUE)
print(dept.plot)
```
...
```{r, echo=FALSE}
sales.no.jewel <- filter(sales.by.dept2, Department != "JEWELRY")
deeper.plot <- ggplot(data = sales.no.jewel) +
  geom_segment(aes(x=1, xend=2, y=`2016`, yend=`2017`)) + 
  geom_text(aes(label=Department, y=`2016`), x=1, hjust=-0.2, check_overlap = TRUE)
print(deeper.plot)

depts.check <- c("JEWELRY", "TOYS", "BOOKS")
for(i in 1:3){
  sales1dept <- filter(sales.by.dept2, Department == depts.check[i])
  title.name <- depts.check[i]
deeper.plot <- NULL
deeper.plot <- ggplot(data = sales1dept) +
  geom_segment(aes(x=1, xend=2, y=`2016`, yend=`2017`)) + 
  geom_text(aes(label=Department, y=`2016`), x=1, hjust=-0.2, check_overlap = TRUE) +
  ggtitle(title.name)
print(deeper.plot)
