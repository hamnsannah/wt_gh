...
data4years <- read.csv("fake.wt.fixed2.csv", stringsAsFactors = FALSE)
...
staffing.day.plot.2 <- function(df, nickname.of.time.range){
  ...
  for(i in 9:23){
    ...
    day.plot <- ggplot(data = conf.df, aes(x=hour)) +
      ...
      geom_ribbon(aes(ymin=estimate-50, ymax=estimate+50), fill="#ef8a62") 
...
## Profile #1 - Time Period XYZ

This first one represents all the very low sales days
